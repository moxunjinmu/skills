...
const CONFIG_PATH = path.join(process.env.HOME || '/Users/moxun', '.openclaw/openclaw.json');
...
function getFeishuCreds() {
  try {
    ...
    const main = accounts.main || accounts.default || {};
    return {
      appId: main.appId || feishu.appId,
      appSecret: main.appSecret || feishu.appSecret,
    };
  } catch (e) {
    return null;
  }
...
async function getToken() {
  ...
  if (!creds || !creds.appId || !creds.appSecret) {
    throw new Error('错误: 未在 openclaw.json 中找到飞书凭证');
  }
  return new Promise((resolve, reject) => {
    ...
    const req = https.request(options, (res) => {
      ...
      res.on('end', () => {
        ...
        if (result.code !== 0) return reject(new Error(`Token error: ${result.msg}`));
...
命令:
  ...
  convert <markdown-file> [output-file]          - 将 Markdown 转换为飞书块格式（不调用 API）

凭证:
  自动从 ~/.openclaw/openclaw.json 读取，无需手动设置环境变量
